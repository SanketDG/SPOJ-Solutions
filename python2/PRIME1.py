...
# T is the number of integer pairs (test cases) and thus times the
# generator will activate
T = input()
...
for t in range(T):
    ...
    # grabs the integer pairs from the user and splits them into two vars, m
    # and n, where there is a space
    m, n = raw_input().split(' ')
    m = int(m)
    n = int(n)
    cap = sqrt(n) + 1

    # if the lower bound is not at least 2, make it 2 (no primes exist smaller
    # than 2)
    if (m < 2):
        m = 2
    ...
    # faster than looping through all the values of isprime and setting them
    # to False individually
    for i in primes:
        if (1 >= cap):
            break
